 ...
 * @author Abir
 */
public class MCODBAII extends Algorithm {
    ...
    public MCODBAII(Problem problem) {
        super(problem);
    }
        ...
        try {
            ...
            if (super.getInputParameter("populationSize") == null) {
                try {
                    ...
                    Logger.getLogger(MCODBAII.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
            ...
            if (memes == null) {
                for (int i = 0; i < referencePoints.size(); i++) {
                    //System.out.println("Reference Points: $$$$$$$$" + i);
                ...
                subPopulations = generateSubPopFromMemes(memes, referencePoints, maxFESubPop);
            }
            ...
            while (UFE < maxFE) {
                ...
                for (Thread subPop : subPopulations) {
                    ...
                    //System.out.println("RandomBestSolution => "+randomlyBestSolution.toString());
                    ...
                    for (int i = 0; i < ((ThreadGA) subPop).getPopulation().size(); i++) {
                        ...
                        //System.out.println(" ===> OffspringsSolution : "+offsprings.get(offsprings.size()-1).toString());
                    }
                    ...
                    /*System.out.println(" newSubPopulations => ");
                        for (SolutionSet newSubPopulation : newSubPopulations) {
                        System.out.println(" ==> : "+newSubPopulation.toString());
                        }*/
                }
    ...
    public ArrayList<Thread> generateSubPopFromMemes(SolutionSet memes, SolutionSet referencesPoints, int maxFESubPop) throws Exception {
        ArrayList<Thread> subPopulations = new ArrayList();
        SolutionSet[] solutionSets = new SolutionSet[referencesPoints.size()];
        for (int i = 0; i < solutionSets.length; i++) {
            solutionSets[i] = new SolutionSet();
        }
        for (int i = 0; i < memes.size(); i++) {
            double min = Double.POSITIVE_INFINITY;
            int pos = 0;
            for (int j = 0; j < referencesPoints.size(); j++) {
                double sumDiff = memes.get(i).sumDiff(referencesPoints.get(j));
                if (sumDiff < min) {
                    min = sumDiff;
                    pos = j;
                }
            }
            solutionSets[pos].add(memes.get(i));
        }
        for (int i = 0; i < solutionSets.length; i++) {
            if (solutionSets[i].size() < popSize / referencesPoints.size()) {
                while (solutionSets[i].size() < popSize / referencesPoints.size()) {
                    Solution solution = new Solution(referencesPoints.get(i));
                    Solution[] solutions = {solution};
                    Object[] neighborhoodResult = neighborhoodOp.execute(solutions);
                    Solution neighborhood = new Solution((Solution) neighborhoodResult[0]);
                    solutionSets[i].add(neighborhood);
                }
            } else if (solutionSets[i].size() > popSize / referencesPoints.size()) {
                while (solutionSets[i].size() > popSize / referencesPoints.size()) {
                    int randomPos = PseudoRandom.randInt(0, solutionSets[i].size());
                    solutionSets[i].remove(randomPos);
                }
            }
            subPopulations.add(new ThreadGA(super.getProblem(), solutionSets[i], maxFESubPop));
        }
