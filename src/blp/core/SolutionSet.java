...
public class SolutionSet implements Serializable {
    ...
    public void printObjectivesToFile(String path) {
        try {
            ...
            System.err.println("Error acceding to the file");
            e.printStackTrace();
        }
    ...
    /**
     * Copies the solutions of the population into array
     *
     * @return A array containing all solution of the population
     */
    public Solution[] writeSolutionSetToArray() {
        if (this.size() == 0) {
            return null;
        }
        Solution[] solutionSet;
        solutionSet = new Solution[size()];
        for (int i = 0; i < size(); i++) {
            solutionSet[i] = get(i);
        }
        return solutionSet;
    } // writeSolutionSetToArray
